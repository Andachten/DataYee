    ...
    def get_prodata(self,smooth=True,tip_correc=True,s=None):
        ...
        for k,v in data.items():
            ...
            if smooth and k == 'retract':
                if s == None:
                    data[k]['vDeflection'] = savgol_filter(data[k]['vDeflection'][:,0], self.data['filters']['win_lens'],self.data['filters']['poly']).reshape(len(data[k]['vDeflection']),1)
                else:
                    data[k]['vDeflection'] = savgol_filter(data[k]['vDeflection'][:,0], s,2).reshape(len(data[k]['vDeflection']),1)
...
class loadjpkfile(forcecurve):
    ...
    def extract_txt_data(self,filename,index):
        ...
        self.data['rawdata']['extend'] = np.array([[tuple(i)] for i in data[:np.argmin(data[:,0])]],dtype=[('measuredHeight', '<f8'), ('vDeflection', '<f8')])
        self.data['rawdata']['retract'] = np.array([[tuple(i)] for i in data[np.argmin(data[:,0]):]],dtype=[('measuredHeight', '<f8'), ('vDeflection', '<f8')])
