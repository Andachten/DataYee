...
class programbody():
    ...
    def drawlabel(self,label,lclplabel):
        ...
        if self.tasktype!='smfs':
            ...
            if self.forcepeak_index<len(self.fc.data['peakindex'])-1:
                lclplabel.setText(' Lc={:.1f}nm; F={:.2f}pN; dLc={:.1f}nm; k={:.1f}'.format(self.fc.data['wlcarg'][self.forcepeak_index][0],data_y[self.fc.data['peakindex'][self.forcepeak_index]][0],self.fc.data['dlc'][self.forcepeak_index],self.fc.data['k'][self.forcepeak_index]))
            elif len(self.fc.data['wlcarg'])>0:
                lclplabel.setText(' Lc={:.1f}nm; F={:.2f}pN; k={:.1f}'.format(self.fc.data['wlcarg'][self.forcepeak_index][0],data_y[self.fc.data['peakindex'][self.forcepeak_index]][0],self.fc.data['k'][self.forcepeak_index]))
    def copypeak_(self):
        if not self.state:
            return None
        if len(self.fc.data['peakindex'])==0:
